...
#define WHIP_RTP_PAYLOAD_TYPE_H264_RTX 98
#define WHIP_RTP_PAYLOAD_TYPE_VP8_RTX 99
#define WHIP_RTP_PAYLOAD_TYPE_VP9_RTX 100
...
typedef struct WHIPContext {
    ...
    uint8_t h264_rtx_payload_type;
    uint8_t vp8_rtx_payload_type;
    uint8_t vp9_rtx_payload_type;
    /**
    ...
    if (whip->video_par) {
        ...
        if (whip->video_par->codec_id == AV_CODEC_ID_H264) {
            ...
            av_bprintf(&bp, ""
                ...
                WHIP_RTP_PAYLOAD_TYPE_H264_RTX,
                WHIP_RTP_PAYLOAD_TYPE_H264_RTX,
                whip->ice_ufrag_local,
                whip->ice_pwd_local,
                whip->dtls_ctx.dtls_fingerprint,
...
static int on_rtp_write_packet(void *opaque, const uint8_t *buf, int buf_size)
{
    ...
    is_video = (payload_type == whip->video_payload_type || 
                payload_type == WHIP_RTP_PAYLOAD_TYPE_H264_RTX ||
                payload_type == WHIP_RTP_PAYLOAD_TYPE_VP8_RTX ||
                payload_type == WHIP_RTP_PAYLOAD_TYPE_VP9_RTX);
    if (!is_rtcp && !is_video && payload_type != whip->audio_payload_type)
        return 0;
