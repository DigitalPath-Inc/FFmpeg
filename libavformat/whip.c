...
#define WHIP_RTP_PAYLOAD_TYPE_VP9 96
...
typedef struct WHIPContext {
    ...
    // Add a new field to store the video codec
...
static int parse_codec(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            whip->video_codec_id = par->codec_id;
...
static int generate_sdp_offer(AVFormatContext *s)
{
    ...
    whip->video_payload_type = (whip->video_codec_id == AV_CODEC_ID_H264) ? 
                               WHIP_RTP_PAYLOAD_TYPE_H264 : WHIP_RTP_PAYLOAD_TYPE_VP9;
    ...
    if (whip->video_par) {
        const char *vcodec_name = (whip->video_codec_id == AV_CODEC_ID_H264) ? "H264" : "VP9";
        ...
        if (whip->video_codec_id == AV_CODEC_ID_H264) {
            av_bprintf(&bp, "a=fmtp:%u level-asymmetry-allowed=1;packetization-mode=1;profile-level-id=%02x%02x%02x\r\n",
                whip->video_payload_type,
                profile,
                profile_iop,
                level);
        } else {
            // Add VP9-specific fmtp line if needed
            av_bprintf(&bp, "a=fmtp:%u profile-id=0\r\n", whip->video_payload_type);
        }
...
static int whip_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    whip->sequence_number = av_get_random_seed() & 0xFFFF;
    whip->ssrc = av_get_random_seed();
    whip->pb = s->pb;  // Assuming s->pb is already set up
...
static int whip_check_bitstream(AVFormatContext *s, AVStream *st, const AVPacket *pkt)
{
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_H264) {
        ...
        // VP9 doesn't need any bitstream filtering for RTP
        ret = 0;
    }
...
const FFOutputFormat ff_whip_muxer = {
    ...
    .p.video_codec      = AV_CODEC_ID_H264, // Keep H264 as default, VP9 will be handled in the code
